...
export function addRootPage() {
  pageStore.update((pages) => {
    const rootChildren = pages.filter(
      (page) => !pages.some((item) => item.connections.includes(page.id)),
    );
...
export function getAllChildPages(pages: Page[], id: PageId): Page[] {
  ...
  const childPages =
    parentPage?.connections.map((itemId) => pages.find((item) => item.id === itemId) as Page) ?? [];
  return childPages.reduce(
    (acc, item) => [...acc, ...getAllChildPages(pages, item.id)],
    childPages,
  );
}
...
export function replaceEmptyParent(childId: PageId) {
  pageStore.update((pages) => {
    ...
    assert(
      !pages.some((item) => item.connections.includes(parent.id)),
      'Parent still exists in pages',
    );
...
export function movePageDown(childId: PageId): Page {
  ...
  pageStore.update((pages) => {
    ...
    const nearestSiblingId =
      index > 0 ? parent.connections[index - 1] : parent.connections[index + 1];
...
function sanitizePages(pages: Page[]): Page[] {
  const filtered = pages.map((page) => {
    ...
    page.connections = Array.from(
      new Set(page.connections.filter((id) => pages.some((item) => item.id === id))),
    );
